...
#import "MyCashCouponIncomeVC.h"
#import "MyCashCounponModel.h"
#import "MyCashCounponCell.h"
@interface MyCashCouponIncomeVC ()

@end

@implementation MyCashCouponIncomeVC
...
{
    //0收入1支出
    NSDictionary *parameters = @{@"cpr_userid":[Single getUserModel].ID, @"cpr_status":@"0"};
    WS(weakSelf)
    [self POST:API_MyCashCoupon parameters:parameters success:^(NSDictionary *dic) {
        ...
        for (MyCashCounponModel* model in weakSelf.listArray) {
            model.isIncome = YES;
        }
    } fail:^(NSError *error) {
