...
static NSString *jsPatchTime = @"JsPatchTime";
...
+(void)loadJSPatch
{
    ...
    NSDate *lastDate = ReadForLocation(jsPatchTime);
    
    if (!lastDate) {
        
        WriteForLocation(myNowDate, jsPatchTime);
    }
    
    if ([myNowDate timeIntervalSinceDate:lastDate] < 3600) {
        return;
    }
