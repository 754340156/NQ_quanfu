...
- (instancetype)init{
    if (self = [super init]) {
        _buttonWidth = Adapted(64);
        _buttonHeight = Adapted(30);
        _paddingHorizontal = Adapted(6);
        _paddingVertical = Adapted(6);
        _paddingBottom = 0;
        _maxViewWidth = 320;
...
- (void)initButtons{
    ...
    [_filterData.elements enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        [button setBackgroundImage:[UIImage imageWithColor:self.buttonNormalColor] forState:UIControlStateNormal];
...
- (void)buttonSelected:(UIButton *)button{
    ...
    if (self.filterData.type == DK_SELECTION_SINGLE) {
        if (button.selected) {
            ...
            [button setBackgroundImage:[UIImage imageWithColor:self.buttonHighlightColor] forState:UIControlStateNormal];
            button.selected = YES;
...
- (void)setSelectedChoice:(NSString *)choice{
    if(self.selectingButton){
        ...
        [self.selectingButton setTitleColor:kWhiteColor forState:UIControlStateNormal];
        [self.selectingButton setBackgroundImage:[UIImage imageWithColor:self.buttonHighlightColor] forState:UIControlStateNormal];
